...
// whose production-key witnesses include every accepted overlay production, and
// compute the output DAG's grammar-level exact parsimony via the B&B trim.
...
inline overlay_chain_compaction_result compact_overlay_chain_to_dag(
  ...
  for (auto topology : options.witness_topologies) {
    overlay_chain_compaction_detail::append_unique_topology(topologies, grammar,
                                                            std::move(topology));
  }
  ...
  for (auto const& key : result.intended_productions) {
    auto dense_pid = overlay_chain_compaction_detail::find_dense_production_by_key(
        grammar, key, "overlay-chain compaction");
    auto topology =
  ...
  if (topologies.empty()) {
    topologies.push_back(first_rank3_topology(grammar));
  }
  ...
  auto merged = merge_rank3_trees_option_a(source, grammar, topologies,
                                           merge_options);
